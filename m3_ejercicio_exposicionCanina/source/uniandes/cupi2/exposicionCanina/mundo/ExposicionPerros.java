...
public class ExposicionPerros
{
		...
		for(int i = perros.size(); i > 0; i--)
		{
			for(int j = 0; j < i-1; j++)
			{
				Perro uno = (Perro) perros.get(j);
                                ...
                                if(uno.darRaza().compareTo(dos.darRaza()) > 0){
                                    perros.set(j, dos);
                                    perros.set(j + 1 , uno);
                                } 
    ...
    public void ordenarPorNombre( )
    {
    	for(int i = 1; i < perros.size(); i++)
		{
			...
			Perro uno = (Perro) perros.get(j);
                        ...
                        if(uno.darNombre().compareTo(dos.darNombre()) < 0){
                            perros.set(j, dos);
                            perros.set(j -1 , uno);
                        } 
                        else
    	...
    	for(int i = perros.size(); i > 0; i--)
		{
			for(int j = 0; j < i-1; j++)
			{
				Perro uno = (Perro) perros.get(j);
                                ...
                                if(uno.darPuntos() > dos.darPuntos()){
                                    perros.set(j, dos);
                                    perros.set(j + 1 , uno);
                                } 
    ...
    public void ordenarPorEdad( )
    {
    	for (int i = 0; i < perros.size() -1; i++){
            Perro menor = (Perro) perros.get(i);
            int posMenor = i;
            for(int j = i+1; j < perros.size(); j++){
                Perro actual = (Perro) perros.get(j);
                if(actual.darEdad() < menor.darEdad()){
                    menor = actual;
                    posMenor = j;
                }
            }
            Perro temp = (Perro) perros.get(i);
            perros.set(i, menor);
            perros.set(posMenor, temp);
